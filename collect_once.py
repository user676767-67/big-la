...
def collect_data():
    # Use Pacific Time for gym hours
    import os
    os.environ['TZ'] = 'America/Los_Angeles'
    
    now = datetime.now()
    print(f"Collection time: {now.strftime('%Y-%m-%d %H:%M:%S')} Pacific")
    ...
    for zone in data:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (now, location_id, location_name, facility_id, facility_name, count, capacity, percentage))
